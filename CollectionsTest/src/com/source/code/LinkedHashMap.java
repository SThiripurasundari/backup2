...
/**
 * <p>
 * Hash table and linked list implementation of the {@code Map} interface, with
 * predictable iteration order. This implementation differs from {@code HashMap}
 ...
 * in which keys were inserted into the map (<i>insertion-order</i>). Note that
 * insertion order is not affected if a key is <i>re-inserted</i> into the map.
 * (A key {@code k} is reinserted into a map {@code m} if {@code m.put(k, v)} is
 * invoked when {@code m.containsKey(k)} would return {@code true} immediately
 ...
 * caches. Invoking the {@code put}, {@code putIfAbsent}, {@code get},
 * {@code getOrDefault}, {@code compute}, {@code computeIfAbsent},
 * {@code computeIfPresent}, or {@code merge} methods results in an access to
 * the corresponding entry (assuming it exists after the invocation completes).
 * The {@code replace} methods only result in an access of the entry if the
 * value is replaced. The {@code putAll} method generates one entry access for
 * each mapping in the specified map, in the order that key-value mappings are
 ...
 *
 * <p>
 * This class provides all of the optional {@code Map} operations, and permits
 * null elements. Like {@code HashMap}, it provides constant-time performance
 * for the basic operations ({@code add}, {@code contains} and {@code remove}),
 * assuming the hash function disperses elements properly among the buckets.
 * Performance is likely to be just slightly below that of {@code HashMap}, due
 * to the added expense of maintaining the linked list, with one exception:
 * Iteration over the collection-views of a {@code LinkedHashMap} requires time
 * proportional to the <i>size</i> of the map, regardless of its capacity.
 * Iteration over a {@code HashMap} is likely to be more expensive, requiring
 ...
 * {@code HashMap}. Note, however, that the penalty for choosing an excessively
 * high value for initial capacity is less severe for this class than for
 * {@code HashMap}, as iteration times for this class are unaffected by
 * capacity.
 *
 * <p>
 ...
 * querying the map with {@code get} is a structural modification. </strong>)
 *
 * <p>
...
public class LinkedHashMap<K, V> extends HashMap<K, V> implements Map<K, V> {
	 ...
	 * structured a little differently. Because superclass HashMap now uses trees
	 * for some of its nodes, class LinkedHashMap.Entry is now treated as
	 * intermediary node class that can also be converted to tree form. The name of
	 * this class, LinkedHashMap.Entry, is confusing in several ways in its current
	 * context, but cannot be changed. Otherwise, even though it is not exported
	 * outside this package, some existing source code is known to have relied on a
	 * symbol resolution corner case rule in calls to removeEldestEntry that
	 * suppressed compilation errors due to ambiguous usages. So, we keep the name
	 * to preserve unmodified compilability.
	 *
	 * The changes in node classes also require using two fields (head, tail) rather
	 * than a pointer to a header node to maintain the doubly-linked before/after
	 * list. This class also previously used a different style of callback methods
	 * upon access, insertion, and removal.
	 */
	...
	/**
	 * The iteration ordering method for this linked hash map: {@code true} for
	...
	/**
	 * Constructs an empty insertion-ordered {@code LinkedHashMap} instance with the
	...
	/**
	 * Constructs an insertion-ordered {@code LinkedHashMap} instance with the same
	 * mappings as the specified map. The {@code LinkedHashMap} instance is created
	...
	/**
	 * Returns {@code true} if this map should remove its eldest entry. This method
	 * is invoked by {@code put} and {@code putAll} after inserting a new entry into
	 * the map. It provides the implementor with the opportunity to remove the
	 * eldest entry each time a new one is added. This is useful if the map
	 * represents a cache: it allows the map to reduce memory consumption by
	 * deleting stale entries.
	 *
	 * <p>
	 * Sample use: this override will allow the map to grow up to 100 entries and
	 * then delete the eldest entry each time a new entry is added, maintaining a
	 * steady state of 100 entries.
	 * 
	 * <pre>
	 ...
	 * the map to modify itself as directed by its return value. It <i>is</i>
	 * permitted for this method to modify the map directly, but if it does so, it
	 * <i>must</i> return {@code false} (indicating that the map should not attempt
	 ...
	 *
	 * <p>
	 * This implementation merely returns {@code false} (so that this map acts like
	 * a normal map - the eldest element is never removed).
	 *
	 ...
	 *               {@code true}. If the map was empty prior to the {@code put} or
	 *               {@code putAll} invocation resulting in this invocation, this
	 ...
	 * {@code add} or {@code addAll} operations. Its {@link Spliterator} typically
	 * provides faster sequential performance but much poorer parallel performance
	 ...
	 * not support the {@code add} or {@code addAll} operations. Its
	 * {@link Spliterator} typically provides faster sequential performance but much
